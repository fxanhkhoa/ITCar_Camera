...
Mat dir_threshold(Mat image, int sobel_kernel);
...
Mat dir_threshold(Mat image, int sobel_kernel = 3){
  ...
  for (int i = 0; i < absgraddir.rows - 1; i++)
    for (int j = 0; j < absgraddir.cols - 1; j++){
      uchar x = sobelx.at<uchar>(i,j);
      uchar y = sobely.at<uchar>(i,j);
