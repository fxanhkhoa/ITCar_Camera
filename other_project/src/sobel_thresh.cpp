...
double *sliding_window(Mat frame) {
  ...
  output = thresholding(frame, 50, 255, 0, 25, 50, 255, 0.7, 1.3, 113, 255, 234,
                        255, 15, 250);
  ...
  for (int i = 0; i < nwindows; i++) {
    int win_y_low, win_y_high, win_xleft_low, win_xleft_high, win_xright_low,
        win_xright_high;
    ...
    rectangle(frame, Point(win_xleft_low, win_y_low),
              Point(win_xleft_high, win_y_high), Scalar(255, 255, 255));
    rectangle(frame, Point(win_xright_low, win_y_low),
              Point(win_xright_high, win_y_high), Scalar(255, 255, 255));
...
double *polyfit(double x[], double y[], int count, int degree) {
  ...
  double B[n + 1][n + 2];
  // voi khai bao a[] phai dung static de dia chi ko bi thay doi dan den segment
  // fault
  // con dung con tro phai new, (contro se co vi tri va return ve dung vi tri
  // do)
  ...
  for (i = 0; i < n; i++) // From now Gaussian Elimination starts(can be
                          // ignored) to solve the set of linear equations
                          // (Pivotisation)
    for (k = i + 1; k < n; k++)
      if (B[i][i] < B[k][i])
        for (j = 0; j <= n; j++) {
  ...
  for (i = 0; i < n; i++)
    cout << "x^" << i << "=" << a[i]
         << endl; // Print the values of x^0,x^1,x^2,x^3,....
...
Mat get_perspective(Mat frame, Mat src) {
  ...
  Point pts_dst[4] = {
      Point(0 + offset, frame.rows), Point(frame.cols / 2.9, frame.rows / 1.9),
      Point(frame.cols / 1.5, frame.rows / 1.9), Point(frame.cols, frame.rows),
  };
